...
```tsx pure title="错误写法 1"
export default function () {
  return <div>Hello World</div>;
}
```

```tsx pure title="错误写法 2"
export default () => <div>Hello World</div>;
```

正确用法是给每个组件函数声明一个名称：

```tsx pure title="正确写法 1"
export function Component1(): React.ReactNode {
  return <div>Hello World</div>;
}
```
```tsx pure title="正确写法 2"
export const Component2: React.FC = () => <div>Hello World</div>;
```
