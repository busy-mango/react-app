...
while IFS=' ' read -r IMAGE_ID IMAGE_NAME CREATED_AT; do
  ...
  if $SKIP; then
    echo -e "\033[32m[Skipping] image $IMAGE_NAME($IMAGE_ID) as exclude\033[0m"
    continue
  fi

  # 检查镜像是否超过 30 天未使用
  if [[ "$CREATED_AT" < "$TARGET_DATE" ]]; then
    echo -e "\033[31m[Deleting] image $IMAGE_NAME($IMAGE_ID) as older\033[0m"
    docker rmi "$IMAGE_ID"
  else
    echo -e "\033[34m[Skipping] image $IMAGE_NAME($IMAGE_ID) as newer\033[34m"
  fi
done <<< "$IMAGES"
